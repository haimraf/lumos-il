...
import {
  Wand2, Sparkles, ChevronRight, Coins, Library, History, Info, ShieldCheck, Flame
} from "lucide-react";
import { useOwlMail } from "@/components/OwlMail"; // הייבוא החשוב!

/**
 * LUMOS IL - OLLIVANDERS V2.7
 * שדרוג: שילוב דואר ינשופים בהצלחה או כישלון ברכישת שרביט.
 */
...
export default function OllivandersPage() {
  ...
  const { sendOwl } = useOwlMail(); // שולפים את פונקציית שליחת ההתראות
  ...
  const handlePurchase = async () => {
    if (!userId || !profile || isPurchasing) return;

    if (profile.galleons < 15) {
      setErrorMsg(`אין לך מספיק גליאונים. נדרשים 15, אך יש לך רק ${profile.galleons}.`);
      sendOwl("כיסך מרוקן...", "אין לך מספיק גליאונים. השלם משימות כדי להרוויח עוד!", "error"); // ינשוף אזהרה
      return;
    }
    ...
    if (error) {
      ...
      sendOwl("הלחש נכשל", "קרתה שגיאה במשרד הקסמים. נסה שנית.", "error");
      return;
    }
    ...
    setTimeout(() => {
      ...
      setIsPurchasing(false);
      sendOwl("השרביט בחר בך!", "השרביט החדש התווסף בהצלחה לתיק האישי שלך.", "magic"); // ינשוף קסום
    }, 3000);
  };
      ...
      <div className="relative w-full max-w-5xl mx-auto px-6 py-10 flex flex-col items-center min-h-screen" dir="rtl">
        ...
        <div className="w-full flex justify-between items-center mb-16">
          <Link href="/dashboard" className="group flex items-center gap-2 text-white/60 hover:text-amber-500 transition-all font-cinzel text-sm tracking-widest font-bold">
            <ChevronRight size={20} className="group-hover:translate-x-1 transition-transform" /> חזרה לטירה
          </Link>
