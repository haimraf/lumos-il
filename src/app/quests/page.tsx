...
import {
  ChevronRight, Coins, Trophy, Sparkles, BookOpen, CheckCircle2, XCircle, Hourglass, Flame
} from "lucide-react";
import { useOwlMail } from "@/components/OwlMail"; // הלחש שקורא לינשופים!

/**
 * LUMOS IL - QUESTS BOARD V2.2
 * שדרוג: חיבור לדואר ינשופים (Toasts) במענה על טריוויה ואיסוף קצבה.
 */
...
export default function QuestsPage() {
  ...
  const { sendOwl } = useOwlMail(); // הנה אנחנו שולפים את הינשוף!
  ...
  const [shuffledSpells, setShuffledSpells] = useState<string[]>([]);
  const today = new Date().toISOString().split('T')[0];

  useEffect(() => {
    setShuffledSpells(shuffleArray(['לומוס', 'אלוהומורה', 'אקספליארמוס', 'פרוטגו']));
    ...
    const fetchProfile = async () => {
      ...
      if (fetchedProfile.last_reward_date === today) setDailyCollected(true);
  ...
  const handleDailyCollect = async () => {
    ...
    if (!error) {
      ...
      // שלחנו ינשוף! 🦉
      sendOwl("קצבה נאספה בהצלחה!", "5 גליאונים נוספו לכיס שלך.", "magic");
    } else {
      sendOwl("תקלה במשרד הקסמים", "לא הצלחנו לאסוף את הגליאונים כעת.", "error");
    }
  ...
  const handleTriviaAnswer = async (answer: string) => {
    ...
    if (!error) {
      if (isCorrect) {
        ...
        // שלחנו ינשוף הצלחה! 🦉
        sendOwl("תשובה נכונה!", "הרווחת 10 נקודות עבור הבית שלך!", "success");
      } else {
        setProfile({ ...profile, last_trivia_date: today });
        setTriviaResult('wrong');
        // שלחנו ינשוף כישלון! 🦉
        sendOwl("טעות בלחש...", "התשובה הנכונה היא אלוהומורה. נסה שוב מחר!", "error");
      }
    }
    setTriviaAnswered(true);
  };
              ...
              <div className="relative z-10">
                {dailyCollected ? (
                  <div className="w-full py-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-cinzel font-bold text-lg flex items-center justify-center gap-3">
                    <CheckCircle2 size={20} /> נאסף להיום
                  </div>
                ) : (
                  <button
                    onClick={handleDailyCollect} disabled={isCollecting}
              ...
              <div className="relative z-10 mt-auto">
                {triviaAnswered ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                    {shuffledSpells.map((spell) => (
                      <button
                        key={spell} onClick={() => handleTriviaAnswer(spell)}
