...
import {
  ...
  ChevronLeft, ChevronRight, LogOut, Settings, Mail, Lock, Sparkles, Zap, Home
} from "lucide-react";
import { useOwlMail } from "@/components/OwlMail";
import MaraudersMap from "@/components/MaraudersMap";

const HOUSE_THEMES: Record<string, any> = {
  Gryffindor: { cardBg: "bg-red-950/10 backdrop-blur-3xl", borderColor: "border-red-500/30", textColor: "text-red-50", accentText: "text-red-500", glowColor: "drop-shadow-[0_0_20px_rgba(239,68,68,0.3)]", nameHe: "גריפינדור", description: "אומץ, תעוזה ואבירות. כאן נבחנת גדולה.", nebula: "bg-red-900/10" },
  Slytherin: { cardBg: "bg-emerald-950/10 backdrop-blur-3xl", borderColor: "border-emerald-500/30", textColor: "text-emerald-50", accentText: "text-emerald-500", glowColor: "drop-shadow-[0_0_20px_rgba(16,185,129,0.3)]", nameHe: "סלית'רין", description: "שאפתנות, פיקחות וטהרת דם. הדרך לפסגה מתחילה פה.", nebula: "bg-emerald-900/10" },
  Ravenclaw: { cardBg: "bg-blue-950/10 backdrop-blur-3xl", borderColor: "border-blue-500/30", textColor: "text-blue-50", accentText: "text-blue-400", glowColor: "drop-shadow-[0_0_20px_rgba(59,130,246,0.3)]", nameHe: "רייבנקלו", description: "חכמה, יצירתיות ולמידה. הראש פתוח לכל תעלומה.", nebula: "bg-blue-900/10" },
  Hufflepuff: { cardBg: "bg-amber-950/10 backdrop-blur-3xl", borderColor: "border-amber-500/30", textColor: "text-amber-50", accentText: "text-amber-500", glowColor: "drop-shadow-[0_0_20px_rgba(245,158,11,0.3)]", nameHe: "הפלפאף", description: "טוב לב, נאמנות ועבודה קשה. הבית של כולם.", nebula: "bg-amber-900/10" }
};
...
function DashboardContent() {
  ...
  // מאזין לקישור כדי לפתוח את ההגדרות אוטומטית
  useEffect(() => {
  ...
  return (
    <div className="relative w-full max-w-7xl mx-auto px-6 py-8 lg:py-12" dir="rtl">
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-start">
        <aside className="w-full lg:w-80 order-2 lg:order-1 shrink-0 animate-in fade-in slide-in-from-right-4 duration-700">
          <div className={`glass-panel rounded-[2.5rem] border-t border-l ${theme.borderColor} p-8 relative overflow-hidden group/sidebar`}>
            <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${theme.colors} opacity-20 blur-[50px] group-hover/sidebar:opacity-40 transition-opacity duration-700 pointer-events-none`}></div>
            ...
            <nav className="flex flex-col gap-2 pt-4">
              <TabButton icon={Zap} label="מרכז הפעילות" active={activeTab === 'overview'} onClick={() => { setActiveTab('overview'); router.push('/dashboard'); }} theme={theme} />
              <TabButton icon={Settings} label="הגדרות חשבון" active={activeTab === 'settings'} onClick={() => { setActiveTab('settings'); router.push('/dashboard?tab=settings'); }} theme={theme} />
        ...
        <div className="flex-1 order-1 lg:order-2 w-full">
          {activeTab === 'overview' ? (
            <div className="space-y-10">
              <section className={`relative overflow-hidden glass-panel rounded-[3rem] border-t border-r ${theme.borderColor} p-10 md:p-16 ${theme.glowColor} group/hero transition-all duration-700 hover:shadow-2xl`}>
                <div className={`absolute -inset-10 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover/hero:opacity-100 transition-opacity duration-1000 pointer-events-none`}></div>
            ...
            <div className={`glass-panel rounded-[3rem] border-t border-r ${theme.borderColor} p-10 md:p-16 space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700`}>
              <div className="flex justify-between items-center mb-8">
                <button onClick={() => { setActiveTab('overview'); router.push('/dashboard'); }} className="flex items-center gap-2 text-white/40 hover:text-white transition-all font-cinzel text-xs font-bold uppercase tracking-widest">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 text-right">
                      <label className="text-[10px] text-white/30 uppercase tracking-widest block mr-2">מגדר</label>
              ...
              <div className={`mt-12 p-8 rounded-[2.5rem] border border-white/10 bg-gradient-to-br from-white/[0.03] to-transparent flex flex-col md:flex-row items-center gap-8 shadow-inner transition-all hover:bg-white/[0.05]`}>
...
// עטיפת Suspense חובה ל-useSearchParams
export default function DashboardPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#020617] flex items-center justify-center"><div className="w-12 h-12 border-t-2 border-amber-500 rounded-full animate-spin"></div></div>}>
      <DashboardContent />
    </Suspense>
  );
}
...
function ActionCard({ href, icon: Icon, title, desc, theme }: any) {
  return (
    <Link href={href} className={`group glass-panel p-10 rounded-[2.5rem] border-t border-r flex flex-col items-center text-center gap-6 relative overflow-hidden`} style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
