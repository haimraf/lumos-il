...
import type { FAQItem } from "@/data/site-features-faq";
...
export default function FAQPageClient({ items }: { items: FAQItem[] }) {
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 md:px-6 py-20">
        <header className="text-center mb-16 space-y-6" aria-labelledby="faq-page-title">
          ...
          <div>
            ...
            <p style={{ fontFamily: "'IM Fell English', serif", fontStyle: "italic", color: "rgba(255,255,255,0.4)", fontSize: "1.15rem", marginTop: "12px" }}>
              כל הלחשים, הסודות, המעברים החבויים והפינות החרישיות של הטירה — מרוכזים כאן
            </p>
          </div>

          <div className="flex items-center justify-center gap-8 pt-2" role="status" aria-live="polite">
            {[
              { value: items.length, label: "שאלות" },
              { value: categories.length, label: "קטגוריות" },
              { value: "∞", label: "קסמים" },
            ].map((stat) => (
              <div key={stat.label} className="text-center">
                <div style={{ fontFamily: "'Cinzel', serif", fontWeight: 900, fontSize: "1.5rem", color: "#fbbf24" }}>{stat.value}</div>
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: "8px", letterSpacing: "0.25em", color: "rgba(255,255,255,0.2)", textTransform: "uppercase" }}>{stat.label}</div>
              </div>
            ))}
          </div>
        </header>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-amber-500/15 bg-white/[0.03] px-4 py-3 shadow-[0_0_30px_rgba(245,158,11,0.06)]">
              ...
              <input
                ...
                placeholder="חפשו לחש, מסדרון, חפץ או שאלה נפוצה"
        ...
        <section className="space-y-2.5" aria-labelledby="faq-results-title">
          ...
          {filtered.map((item) => {
            ...
            return (
              <div key={itemId} className={`parchment-card rounded-2xl overflow-hidden ${isOpen ? "open" : ""}`}>
                <button
                  type="button"
