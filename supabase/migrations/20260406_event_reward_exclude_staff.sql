-- Exclude staff/management roles from event leaderboard and reward distribution.
-- These roles match the SORTING_BYPASS_ROLES list defined in src/lib/profileAccess.ts.
...
DECLARE
    ...
    rewards_already_distributed BOOLEAN := false;
BEGIN
    PERFORM pg_advisory_xact_lock(hashtext('public.distribute_event_rewards')::bigint);
    ...
    rewards_already_distributed := COALESCE((event_settings->>'rewards_distributed')::boolean, false)
        OR NULLIF(COALESCE(event_settings->>'completed_at', ''), '') IS NOT NULL;

    IF rewards_already_distributed THEN
        RAISE EXCEPTION 'Event rewards already distributed for %', event_label;
    END IF;
    ...
    FOR winner_row IN
        SELECT
            id,
            full_name,
            GREATEST(COALESCE(event_points, 0), COALESCE(passover_points, 0)) AS total_points
        FROM public.profiles
        WHERE GREATEST(COALESCE(event_points, 0), COALESCE(passover_points, 0)) > 0
          AND COALESCE(role, '') NOT IN (
              'מייסד', 'מייסדת', 'ראש הוגוורטס', 'שומר הטירה',
              'פרופסור', 'צוות Lumos', 'מנהל', 'מנהלת', 'מנחה', 'הנהלה'
          )
        ORDER BY GREATEST(COALESCE(event_points, 0), COALESCE(passover_points, 0)) DESC,
    ...
    LOOP
        ...
        notification_content := format(
            '🏆 סיימת במקום %s באיוונט %s וקיבלת %s גליאונים!',
            rank_count,
            event_label,
            galleons_reward
        );

        IF house_points_reward > 0 THEN
            notification_content := notification_content || format(' ו-%s נקודות בית!', house_points_reward);
        END IF;
        ...
        IF NOT EXISTS (
            ...
            WHERE user_id = winner_row.id
              AND type = 'achievement'
              AND target_url = event_target_url
              AND content = notification_content
        ) THEN
