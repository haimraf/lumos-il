-- Restore the correct distribute_event_rewards function.
-- Background: 20260325_event_system_sync.sql ran after all event_reward_* migrations
-- alphabetically and silently replaced the improved function with an older version
-- that lacked target_url in notifications, has_configured_rewards logic, and used a
-- fake UUID for actor_id that may violate FK constraints.
-- This migration re-applies the final correct version.
...
BEGIN
    ...
    LOOP
        ...
        -- When rewards are explicitly configured, skip ranks with no reward entry.
